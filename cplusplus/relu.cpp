...
int main() {
    ...
    PerceptronMultilayer nn(num_features_int,capas_ocultas_int,neuronas_por_capa_int,1,"relu");
    nn.train(x_train,y_train,0.0001,1000);
    vector<vector<double>> predictions= nn.predict(x_test);
    std::stringstream ss;
    ss << "../output_relu/predictions" << capas_ocultas << "_" << neuronas_por_capa << ".txt";
